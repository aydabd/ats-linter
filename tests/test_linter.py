import pytest
from ats_linter.linter import ATSTestCase, ATSTestCasesFactory, LintTestCase, ATSTestCasesLinter
from ats_linter.data_classes import TestCase
from ats_linter.description import SECTION_OBJECTIVE, SECTION_APPROVALS, SECTION_TEST_STEPS
...
def test_linttestcase_missing_sections():
    # Missing approvals
    docstring = make_docstring({SECTION_OBJECTIVE: "obj", SECTION_TEST_STEPS: "1. Do something\n2. Verify that step 1"})
    tc = TestCase(name="test_missing", docstring=docstring, code="def test_missing(): pass")
    ats = ATSTestCase(tc)
...
def test_linttestcase_mismatched_approvals_and_steps():
    ...
    tc = TestCase(name="test_mismatch", docstring=docstring, code="def test_mismatch(): pass")
    ats = ATSTestCase(tc)
    linter = LintTestCase(ats)
    assert linter.lint() is False
...
def test_check_sections_with_none():
    from ats_linter.linter import LintTestCase, MANDATORY_SECTIONS
    class DummySection:
    ...
    linter.test_description = type("Dummy", (), {s.lower().replace(" ", "_"): True for s in MANDATORY_SECTIONS})()
...
def test_atstestcaseslinter_lint_all_pass(monkeypatch):
    ...
    try:
        tc1 = TestCase(name="test_foo", docstring="Objective:\nfoo", code="def test_foo(): pass")
...
def test_atstestcaseslinter_lint_multiple_workers(monkeypatch):
    ...
    try:
        tc1 = TestCase(name="test_foo", docstring="Objective:\nfoo", code="def test_foo(): pass")
        tc2 = TestCase(name="test_bar", docstring="Objective:\nbar", code="def test_bar(): pass")
        ats1 = ATSTestCase(tc1)
