import pytest
from ats_linter.data_classes import TestCase, TestModule, TestClass
...
def test_testclass_len_and_dict():
    ...
    test_class = TestClass(name="A", docstring="docA", test_cases=[tc1, tc2], fixtures=[])
...
def test_testmodule_len_and_dict():
    ...
    mod = TestModule(name="mod1", test_classes=[test_class], test_cases=[tc2], fixtures=[])
    assert len(mod) == 2
    d = mod.__dict__()
